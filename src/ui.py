...
from PyQt6.QtWidgets import (
    ...
    QMainWindow, QMenuBar, QMessageBox, QProgressBar, QPushButton, QScrollArea, QSizePolicy,
    QStackedWidget, QVBoxLayout, QWidget,
)

from src.config import ThemeConfig
from src.core import AppInfo, AppType, VersionInfo
from src.launcher import AppLauncher, is_installed, _start_dialog_accepter
...
class AppListScreen(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # Driver install button — always at the bottom
        driver_btn = QPushButton("🔧  Cài đặt Driver")
        driver_btn.setFont(QFont(self._theme.font_family, self._theme.font_size, QFont.Weight.Bold))
        driver_btn.setFixedHeight(40)
        driver_btn.setFixedWidth(480)
        driver_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        driver_btn.setStyleSheet(
            f"QPushButton {{"
            f"  color: {self._theme.text_color};"
            f"  background: qlineargradient(x1:0,y1:0,x2:1,y2:0,"
            f"    stop:0 #1a2a1a, stop:1 #0d3320);"
            f"  border: 1px solid #2ecc71;"
            f"  border-radius: 8px;"
            f"}}"
            f"QPushButton:hover {{"
            f"  background: qlineargradient(x1:0,y1:0,x2:1,y2:0,"
            f"    stop:0 #1e3d1e, stop:1 #145228);"
            f"  border: 1px solid #27ae60;"
            f"  color: #ffffff;"
            f"}}"
            f"QPushButton:pressed {{ background: #145228; }}"
        )
        driver_btn.clicked.connect(self.install_drivers_requested)
        layout.addWidget(driver_btn, alignment=Qt.AlignmentFlag.AlignHCenter)
...
class _DriverInstallWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            entries = sorted(os.scandir(self._drivers_dir), key=lambda e: e.name)
        except Exception as exc:
            self.finished.emit(False, str(exc))
            return

        for entry in entries:
            # Sub-folder containing DPInst driver package (e.g. ftdi/)
            if entry.is_dir():
                dpinst = os.path.join(entry.path,
                    "DPInstx64.exe" if is_64bit else "DPInstx86.exe")
                if not os.path.isfile(dpinst):
                    dpinst = os.path.join(entry.path, "DPInst.exe")
                if os.path.isfile(dpinst):
                    logging.warning("Running DPInst: %s", dpinst)
                    r = subprocess.run(
                        [dpinst, "/s", "/sw"],
                        timeout=120,
                    ...
                    logging.warning("DPInst exit=%d", r.returncode)
                    # 0=success, 1=reboot, 3010=reboot needed, 512=no matching devices (driver still installed to store)
                    ok = r.returncode in (0, 1, 3010, 512, 0x200)
                    results.append(f"✓ {entry.name}" if ok else f"⚠ {entry.name} (exit {r.returncode})")
                    continue

                # Fallback: pnputil with .inf files in subfolder
                import glob as _glob
                for inf_file in _glob.glob(os.path.join(entry.path, "*.inf")):
                    r = subprocess.run(
                        ["pnputil", "/add-driver", inf_file, "/install"],
                        timeout=60, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                        creationflags=0x08000000,
                    )
                    logging.warning("pnputil %s exit=%d", os.path.basename(inf_file), r.returncode)
                    if r.returncode in (0, 259):
                        results.append(f"✓ {entry.name}/{os.path.basename(inf_file)}")
                continue

            # Direct .exe in Drivers/ root — skip FTDI SFX wrapper (shows console)
            if not entry.is_file() or not entry.name.lower().endswith(".exe"):
                continue
            if "ftdi" in entry.name.lower():
                results.append(f"⚠ {entry.name} (skipped — use Drivers/ftdi/ subfolder instead)")
                continue

            # Generic driver exe
            stop = _start_dialog_accepter()
            try:
                for flags in ["/s", "/S", "/silent", "/q"]:
                    proc = subprocess.run(
                        [entry.path, flags], timeout=120,
                        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                        creationflags=0x08000000,
                    )
                    logging.warning("Driver %s exit=%d", entry.name, proc.returncode)
                    if proc.returncode in (0, 1, 3010):
                        results.append(f"✓ {entry.name}")
                        break
                else:
                    results.append(f"⚠ {entry.name} (exit {proc.returncode})")
            except subprocess.TimeoutExpired:
                results.append(f"✗ {entry.name} (timeout)")
            except Exception as e:
                results.append(f"✗ {entry.name} ({e})")
            finally:
                stop.set()

        if not results:
            self.finished.emit(False, "Không tìm thấy file driver nào trong thư mục Drivers/")
            return
        self.finished.emit(True, "Đã cài đặt:\n" + "\n".join(results))
...
class MainWindow(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        menubar.setFixedHeight(36)
        menubar.setStyleSheet("""
            QMenuBar {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #0d1b3e, stop:1 #1a1a2e);
                color: #c0c8e0;
            ...
            QMenu::item:selected {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(233,69,96,0.25), stop:1 rgba(233,69,96,0.05));
                color: #ffffff;
        ...
        # --- Thông tin ---
        info_menu = menubar.addMenu("ℹ Thông tin")

        about_action = QAction("🏎 Giới thiệu", self)
        about_action.triggered.connect(self._show_about)
        info_menu.addAction(about_action)

        contact_action = QAction("💬 Liên hệ", self)
        contact_action.triggered.connect(self._open_contact)
        info_menu.addAction(contact_action)
    ...
    def _show_about(self) -> None:
        ...
        banner.setStyleSheet("""
            background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                stop:0 #0d1b3e, stop:0.6 #1a1a2e, stop:1 #2a0a14);
        ...
        title_lbl = QLabel("Dyno All In One")
        title_lbl.setFont(QFont("Segoe UI", 16, QFont.Weight.Bold))
        title_lbl.setStyleSheet("color: #e94560; background: transparent; letter-spacing: 1px;")
        ...
        def _row(label: str, value: str, link: str = "") -> None:
            row = QHBoxLayout()
            lbl = QLabel(label)
            lbl.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
            lbl.setStyleSheet("color: #8899bb; background: transparent;")
            lbl.setFixedWidth(130)
            val = QLabel(value)
            val.setFont(QFont("Segoe UI", 10))
            if link:
                val.setStyleSheet("color: #81ecff; text-decoration: underline; background: transparent;")
                val.setCursor(Qt.CursorShape.PointingHandCursor)
                val.mousePressEvent = lambda e, u=link: __import__("webbrowser").open(u)
            else:
                val.setStyleSheet("color: #e0e0e0; background: transparent;")
            row.addWidget(lbl)
            row.addWidget(val)
            row.addStretch()
            body_layout.addLayout(row)

        _row("Tên sản phẩm:", "Dyno All In One")
        _row("Phiên bản:", "1.0")
        _row("Người sáng lập:", "Khoa Lê")
        _row("Trang chủ:", "dynoaio.vercel.app", "https://dynoaio.vercel.app")
        ...
        body_layout.addWidget(ok_btn, alignment=Qt.AlignmentFlag.AlignRight)
        layout.addWidget(body)
        dlg.exec()

    def _open_contact(self) -> None:
        import webbrowser
        webbrowser.open("https://www.facebook.com/lee.khoa257")

    def _check_update(self, silent: bool = False) -> None:
        """Kiểm tra cập nhật. silent=True thì chỉ hiện dialog nếu có bản mới."""
        from src.updater import check_app_update, download_and_replace_exe, CURRENT_VERSION
        import subprocess

        latest, download_url = check_app_update(CURRENT_VERSION)

        if not latest:
            if not silent:
                QMessageBox.information(self, "Kiểm tra cập nhật",
                                        f"Bạn đang dùng phiên bản mới nhất (v{CURRENT_VERSION}).")
            return

        msg = QMessageBox(self)
        msg.setWindowTitle("Có bản cập nhật mới")
        msg.setText(f"🚀  Phiên bản mới: <b>v{latest}</b>")
        msg.setInformativeText(
            f"Phiên bản hiện tại: v{CURRENT_VERSION}\n\n"
            "Thư mục Apps và Drivers sẽ không bị ảnh hưởng.\n\n"
            "Bạn có muốn cập nhật ngay không?"
        )
        msg.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        msg.setDefaultButton(QMessageBox.StandardButton.Yes)
        msg.button(QMessageBox.StandardButton.Yes).setText("Cập nhật ngay")
        ...
        if not download_url:
            import webbrowser
            webbrowser.open("https://github.com/kakalotvz/dynoaio/releases/latest")
            return

        # Hiện loading và tải về
        self._loading.show_blocking("install")
        self._loading._title_lbl.setText("ĐANG TẢI CẬP NHẬT")
        self._loading._sub_lbl.setText(f"Đang tải phiên bản v{latest}...")
        QApplication.processEvents()
        ...
        def _progress(downloaded: int, total: int) -> None:
            if total > 0:
                ...
                self._loading._sub_lbl.setText(
                    f"Đang tải... {pct}% ({downloaded // 1024 // 1024}MB / {total // 1024 // 1024}MB)"
                )
                QApplication.processEvents()

        success, result = download_and_replace_exe(download_url, on_progress=_progress)
        self._loading.hide()

        if not success:
            QMessageBox.warning(self, "Cập nhật thất bại", f"Không thể tải bản cập nhật:\n{result}")
            return

        # result là đường dẫn batch script — chạy nó rồi thoát app
        bat_path = result
        QMessageBox.information(
            self, "Cập nhật thành công",
            f"Đã tải phiên bản v{latest}.\nỨng dụng sẽ khởi động lại để hoàn tất cập nhật."
        )
    ...
    def _apply_theme(self) -> None:
        ...
        self.setStyleSheet(f"background-color: {self._theme.background_color};")
        # Kiểm tra cập nhật tự động khi khởi động (silent — chỉ hiện nếu có bản mới)
    ...
    def _on_install_drivers(self) -> None:
        drivers_dir = _resource_path("Drivers")
        if not os.path.isdir(drivers_dir):
            QMessageBox.warning(self, "Không tìm thấy",
                                "Thư mục Drivers/ không tồn tại.\n"
                                "Hãy tạo thư mục Drivers/ và đặt các file driver (.exe) vào đó.")
            return
        if self._driver_worker and self._driver_worker.isRunning():
            return
        self._loading.show_blocking("install")
        self._driver_worker = _DriverInstallWorker(drivers_dir)
        self._driver_worker.finished.connect(self._on_driver_install_done)
        self._driver_worker.start()

    def _on_driver_install_done(self, success: bool, message: str) -> None:
        self._loading.hide()
        if success:
            QMessageBox.information(self, "Cài đặt Driver", message)
        else:
